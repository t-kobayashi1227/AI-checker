# Project: AI レベルチェック＆ユースケースサイト

## サービス概要

| 機能 | URL | 説明 |
|------|-----|------|
| AIチェッカー | `/` `/quiz` `/result` | 10問4択クイズでAIレベルを診断 |
| ユースケース | `/usecases` `/usecases/[slug]` | AIビジネス活用の読み物コンテンツ |
...
### ユースケースページ
AIのビジネス活用事例・導入ヒントを提供する読み物コンテンツ。
Notion で管理し、Supabase 経由でサイトに表示する。
...
| `docs/usecases-design.md` | ユースケースページ設計書（記事管理・同期・スキル設計） |
